...
func createFileTable(db *sql.DB) error {
	query := `CREATE TABLE IF NOT EXISTS files(ID BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY, host varchar(32) not null, path varchar(1023) not null, name varchar(255) not null, extension varchar(255), hash varchar(255), size BIGINT, created DATETIME, modified DATETIME, accessed DATETIME, birth DATETIME);`
